...
| Supply mechanism | How a session received direction: `runner` (owned OpenCode server, attached verbatim), `hook` (host hook supplied, recorded without duplication), `none` (loader missing/failed, reason recorded, job continues). The ledger records what was actually sent, with the direction hash in every injected case. | direction source |
