...
| T3 path | The execution path a job takes when submitted with `--planner-t3-thread`: dispatcher and worker turns run as child threads on a T3 server, and questions and the terminal report are messages in the planner thread. Jobs without it take the direct CLI path. | T3 mode |
| Planner thread | The T3 thread hosting the planner that submitted a job on the T3 path; its children are the job's dispatcher and worker threads, and it receives the job's questions and terminal report. | parent session |
| Child thread | A T3 thread running one job slot (the dispatcher, or one worker seq) under the planner thread, identified as `sub.<planner>.<suffix>` until the fork's parent link lands. | subthread |
