...
| Invocation | One recorded run for a job: a T3 thread turn (dispatch, worker, correction, recovery, review), a planner question or terminal post, or a verification (proof) attempt. Agent Observer's *dispatch* plus *attempt*. | call, run |
...
| Stage | A step of the flow with an executor and an ordered route list: dispatch, the implementation lanes, critical, correction, recovery, review. | phase, step |
